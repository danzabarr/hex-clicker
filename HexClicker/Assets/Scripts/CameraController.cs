...
public class CameraController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        pitch.localRotation = Quaternion.Euler(zoomAmount * 45 + 45, pitch.localRotation.eulerAngles.y, pitch.localRotation.eulerAngles.z);
        camera.transform.localPosition = Vector3.forward * -(zoomAmount * 22 + 3);

        zoomVelocity *= Mathf.Pow(zoomDampening, Time.deltaTime);
        depthOfField.focusDistance.value = Mathf.Min(zoomAmount * 17 + 2, 20);
        depthOfField.focalLength.value = 10 + (zoomAmount * 22 + 2) * 10;
    }
}
