...
namespace HexClicker.Noise
{
    public class Simplex
    {
        ...
        {
            Simplex noise = new Simplex(seed + "");
            ...
            for (int y = 0; y < resolution + 1; y++)
            {
                for (int x = 0; x < resolution + 1; x++)
                {
                    ...
                    float sample = Noise(noise, point.x, point.y, 0, settings);
        ...
        public static float Noise(Simplex noise, float x, float y, float z, NoiseSettings settings)
        {
            return Noise(noise, x, y, z, settings.frequency, settings.octaves, settings.lacunarity, settings.persistence);
        }
        public static float Noise(Simplex noise, float x, float y, float z, float frequency, int octaves, float lacunarity, float persistence)
        {
