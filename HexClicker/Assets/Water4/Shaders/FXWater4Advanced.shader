...
CGINCLUDE
	...
	v2f vert(appdata_full v)
	{
		...
		half3 vtxForAni = worldSpaceVertex.xzz - _WorldOffset;
		...
		half2 tileableUv = mul(unity_ObjectToWorld,(v.vertex)).xz - _WorldOffset.xz;
	...
	half4 frag( v2f i ) : SV_Target
	{
		...
		if (LinearEyeDepth(refrFix) < i.screenPos.w)
			rtRefractions = rtRefractionsNoDistort;
		#endif
		
		half3 reflectVector = normalize(reflect(viewVector, worldNormal));
		half3 h = normalize ((-_WorldSpaceLightPos0.xyz) + viewVector.xyz);
	...
	v2f_noGrab vert300(appdata_full v)
	{
		...
		o.worldPos = worldSpaceVertex - _WorldOffset;
		...
		// one can also use worldSpaceVertex.xz here (speed!), albeit it'll end up a little skewed
		half2 tileableUv = mul(unity_ObjectToWorld,v.vertex).xz - _WorldOffset;
		o.bumpCoords.xyzw = (tileableUv.xyxy + _Time.xxxx * _BumpDirection.xyzw) * _BumpTiling.xyzw;
	...
	v2f_simple vert200(appdata_full v)
	{
		...
		half3 worldSpaceVertex = mul(unity_ObjectToWorld, v.vertex).xyz;
		half2 tileableUv = worldSpaceVertex.xz - _WorldOffset;
	...
	half4 frag200( v2f_simple i ) : SV_Target
	{
		...
		baseColor.rgb += spec * _LightColor0;
...
Subshader
{
	...
	Pass {
		...
		half4 pointLightFrag( v2f i ) : SV_Target
		{
			half3 worldNormal = PerPixelNormal(_BumpMap, i.bumpCoords, VERTEX_WORLD_NORMAL, PER_PIXEL_DISPLACE);
			/*
			half3 viewVector = normalize(i.viewInterpolator.xyz);
			...
			*/
			...
			float3 worldPos = i.worldPos + _WorldOffset;
			float3 viewDir = normalize(_WorldSpaceCameraPos - worldPos);
			half3 bumpedNormal = worldNormal;
			...
			float4 result = UNITY_BRDF_PBS(
				...
				CreateLight(worldPos, bumpedNormal), indirectLight
			);
